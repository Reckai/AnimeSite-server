...
async function createOrConnectGenres(genres: any[]) {
  return Promise.all(
...
async function createOrConnectStudios(studios: any[]) {
  return Promise.all(
    studios.map(async (studio) => {
      const existingStudio = await prisma.studio.upsert({
        where: { name: studio.name as string },
        update: {},
        create: {
          name: studio.name
        }
      });
      return { id: existingStudio.id };
    })
  );
}
const animeList  = [
...
async function prepareAnimeData(animeList: any[]): Promise<Prisma.AnimeCreateInput[]> {
  return Promise.all(
    animeList.map(async (anime) => {
      const genreConnections = await createOrConnectGenres(anime.genres);
      const studioConnections = await createOrConnectStudios(anime.studios);
...
async function main() {
  try {
    ...
    const preparedAnimeData = await prepareAnimeData(animeList);
    
    for (const animeData of preparedAnimeData) {
